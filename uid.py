...
def fetch_player_info(uid):
    """Auto-detect region and fetch player info"""
    # List of common regions to scan
    regions = ["ind", "br", "us", "sg", "ru", "me", "pk", "id", "tw", "vn", "th", "eu"]
    ...
    for region in regions:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Check if data actually contains a nickname (means player found)
                if data and "basicInfo" in data and data["basicInfo"].get("nickname"):
                    return {"success": True, "data": data, "region": region}
        except:
            continue # Try next region if error
            
    return {"success": False, "message": "Player not found in any common region."}

@app.route('/')
def home():
    return jsonify({
        "success": True,
        "message": "Free Fire Level Info API (Auto-Region Support)",
        "endpoints": {
            "/level?uid=<uid>": "Get level progress (Auto-detects region)",
            "/levels": "Get all level EXP requirements"
        },
        "credit": "@RE_TOXIC"
    })

# ===== Modified Endpoint: Now Auto-Checks Region =====
@app.route('/level')
def get_level_info():
    """Get basic level information using query params: /level?uid=123"""
    uid = request.args.get('uid')

    if not uid:
        return jsonify({
            "success": False,
            "message": "Missing required parameter: uid is required."
        }), 400
    ...
    try:
        ...
        detected_region = player_data["region"]
